...
curl -sL 'https://api.github.com/repos/CTCaer/hekate/releases/latest' \
    ...
    | xargs -n2 sh -c 'curl -L https://ghproxy.com/$1 -o downloads/$0'
...
curl -sL 'https://api.github.com/repos/528787067/SwitchPackage/tags' \
    | jq -r '.[0] | .name, .zipball_url' \
    | xargs -n2 sh -c 'curl -L $1 -o downloads/SwitchConfigs_$0.zip'
find 'downloads' -name 'SwitchConfigs_*.zip' | xargs -I {} unzip -q -u -o -d 'packages' {}
cp -r -f packages/*-SwitchPackage-*/configs/* 'packages'
rm -r -f packages/*-SwitchPackage-*
